...
def data_prep_lift(data_for_lift, merged_df):
    # List of columns used for feature combinations
    ...
    # Step 1: Generate feature combinations from each row in merged_df
    feature_combinations = []

    for idx, row in merged_df.iterrows():
        # For each row, we extract the columns with non-null values
        non_null_columns = row[columns_to_combine].dropna().index.tolist()

        # Step 2: Generate combinations of non-null columns for this row
        # The size of the combination is the number of non-null columns in the row
        if len(non_null_columns) > 1:  # Only consider rows with more than one non-null value
            row_combinations = list(combinations(non_null_columns, len(non_null_columns)))  # Combinations with the size of all non-null columns
            feature_combinations.extend(row_combinations)

    # Step 3: Calculate cancer probabilities using the correct dataframe
